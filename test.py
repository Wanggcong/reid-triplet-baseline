...
parser.add_argument('--name', default='ft_ResNet50_4', type=str, help='save model path')
...
def extract_feature(model,dataloaders):
    ...
    for data in dataloaders:
        ...
        if opt.use_dense:
            ...
            ff = torch.FloatTensor(n,128).zero_()
        for i in range(2):
            if(i==1):
                img = fliplr(img)
...
# gallery_cam,gallery_label = get_id2(gallery_path,1)
# query_cam,query_label = get_id2(query_path,2)
...
if opt.use_dense:
    ...
    print('***************************resnet************************')    
model_structure = nn.DataParallel(model_structure).cuda()
